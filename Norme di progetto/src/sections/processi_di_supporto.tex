...
\item Se vi è un concetto da spiegare esso viene scritto in grassetto seguito da \textbf{':'} e segue la spiegazione di esso, la quale deve iniziare in maiuscolo.
\end{itemize}
...
Per la gestione dei file e gli aggiornamenti dei documenti viene utilizzato il sistema di controllo di versione Git, attraverso il servizio di GitHub. Per la scrittura dei verbali interni ed esterni si è deciso di utilizzare il servizio Confluence offerto da JIRA.
...
Confluence, un servizio fornito da JIRA è utilizzato dal gruppo per la scrittura e l'organizzazione di:
\begin{itemize}
	\item \textbf{Verbali}: Interni ed esterni;
	\item \textbf{Sprint retrospective}: Contenente le varie analisi retrospettive del gruppo sugli sprint svolti;
...
Per evitare i conflitti tra le modifiche, mantenere in ordine i file e garantire che all'interno del branch principale ci siano solo documenti verificati, il gruppo ha deciso che ogni qual volta sia necessaria una modifica in uno specifico documento deve essere creato un branch per apportarla. La modifica in questione potrà essere integrata nel branch principale solo dopo essere stata revisionata dal \textit{Ve}.
